...
### Q6. What are the Message Batches API's actual trade-offs, and when is it the right choice?
...
### Q7. How does prompt caching actually work, and what does "ordering for a stable prefix" mean in practice?
...
### Q8. How do you actually enforce structured, machine-parseable output from a model, and why doesn't "ask nicely" work reliably?
...
### Q9. When does an agent's autonomy actually earn its added complexity, versus when should it be a fixed workflow?
...
### Q10. What problem do subagents/delegation solve, and why does "isolating context" matter?
...
new version against your actual eval suite (Q22) before switching, on your own timeline, rather
...
### Q12. How is prompt injection actually defeated, structurally — and why doesn't "tell the model to ignore injected instructions" work reliably?
...
### Q13. Why does least privilege on tool access beat confirmation dialogs and logging as a security control?
...
### Q14. What's the trap with granting a coding agent broad, bypass-permissions access instead of scoping it?
...
on a wrong assumption, or on injected instructions from untrusted content it read (Q12), has no
...
### Q15. When do you build a custom tool, an MCP server, a Skill, or rely on a built-in capability?
...
### Q16. Why do overlapping, vague tool descriptions cause an agent to pick the wrong tool — and how do you fix it?
...
### Q17. What is "context hygiene," and why doesn't a bigger context window solve the underlying problem it addresses?
...
exploratory/verbose subtasks into subagents (Q10) rather than letting their full output accumulate
...
### Q18. What does context compaction actually do in a long agent session, and what shouldn't you assume it preserves?
...
### Q19. What do few-shot examples actually fix, that adding more instructions doesn't?
...
out and dilute the ones that matter most (the same context-dilution dynamic as Q17). The practical
...
### Q20. How do you diagnose whether a wrong answer from a RAG-based system is a retrieval defect or a model defect?
...
### Q21. What does it mean for an AI system to need a human in the loop, and how do you decide where to put that gate?
...
least-privilege scoping (Q13) rather than relying on the confirmation step alone as the only
...
### Q22. How do you actually evaluate an LLM-powered application's quality, and why is "it seemed to work when I tried it" not sufficient?
...
### Q23. What does audit logging and observability actually need to capture for an AI agent system, beyond what a typical web service logs?
...
diagnosis in Q20 possible at all — without it, there's no way to see what was actually retrieved and
...
### Q24. Design the tool-permission model for a coding agent that has access to a codebase and the ability to deploy to production. What's autonomous versus gated?
**Answer:** Apply least privilege (Q13) scoped by reversibility and blast radius, the same lens used
...
explicit confirmation (Q14); and deploying to production — the highest blast-radius, often-hard-to-
...
of an agent) or a flat "trust it, it's usually right" model (Q14's exact risk) — the actual design
...
### Q25. A RAG-based internal support bot is giving confidently wrong answers to a subset of questions. Walk through diagnosing it.
**Answer:** Start exactly at Q20's diagnostic split: for a representative sample of the wrong-answer
...
buried among too much irrelevant material (context dilution, Q17) or whether it's a genuine
...
### Q26. When does a multi-agent architecture actually pay for its added complexity, versus a single agent with a well-designed toolset?
**Answer:** Multi-agent architectures earn their complexity when a task genuinely decomposes into
substantially independent subtasks that benefit from context isolation (Q10) — each subagent
working with its own focused context rather than one agent's context accumulating everything from
every subtask, which both degrades quality (context rot, Q17) and makes the overall session harder
...
### Q27. How do you evaluate and choose a model for a new AI feature, and how do you manage model version upgrades over its production life?
...
product — run the actual candidate models against a representative eval suite (Q22) for this
...
- **Diagnosis:** This is a prompt injection (Q12) — the processed content contained instructions
...
- **Resolution:** Immediately audit and revoke the tool access the injection exploited for this
  ...
  processing untrusted external content (Q12's actual defense).
...
- **Diagnosis:** Check session traces for runaway tool-calling loops (S1), context that's grown
  very large from accumulated, unpruned tool output over a long session (Q17), or a workflow that
  was built as a fully autonomous agent for what's actually a bounded, latency-tolerant bulk task
  that would have been both cheaper and more predictable as a fixed workflow (Q9), or run via the
  Batches API (Q6) if genuinely bulk and non-interactive.
- **Example:**
  ```
...
- **Resolution:** Pin to the specific prior model version immediately to restore known behavior
  while the new version is properly evaluated, run the new version against the eval suite (Q22)
...
- **Prevention:** Never reference "latest" in production — pin explicit model versions everywhere,
  ...
  (Q27), not something to adopt automatically the moment it's available.
...
- **Diagnosis:** Confirmed by Q16 — the tool descriptions don't clearly differentiate when each
...
- **Diagnosis:** Confirms Q8's point directly — free-text instruction to produce JSON is not a
...
- **Diagnosis:** This is context rot (Q17) — the actually-relevant information for the current
...
- **Resolution:** Introduce context management for long sessions specifically — periodically
  ...
  subagents (Q10) so their exploratory detail doesn't accumulate in the main session's context at
  all.
...
- **Diagnosis:** Per Q18, compaction preserves the gist of a session, not every detail verbatim —
...
- **Diagnosis:** Following Q20/Q25's diagnostic method — pull the actual retrieved context for a
...
- **Example:**
  ...
  ```
- **Resolution:** Fix the retrieval gap if that's what the trace shows (Q20's chunking/embedding/
...
- **Prevention:** Include "does this response appropriately express uncertainty when the retrieved
  context doesn't support a confident answer" as an explicit eval criterion (Q22) from the start,
...
- **Diagnosis:** Regardless of the specific proximate trigger, the fact that the action was
  *possible at all* traces back to a least-privilege violation (Q13) — the agent held a
...
- **Diagnosis:** The agent was running in a bypass-permissions/auto-accept mode (Q14) on a shared
...
- **Diagnosis:** Check the actual prompt structure for what's placed first — a common cause is
  ...
  than after it (Q7) — since caching matches on a shared prefix, anything variable placed early
...
- **Symptoms:** A task split across several subagents (Q10, Q26) completes, but its combined
...
- **Diagnosis:** The subagents' context isolation, which is exactly what makes delegation valuable
  for keeping each one's context lean (Q10), has also isolated them from information they actually
...
- **Resolution:** Identify the specific shared state/decisions causing the inconsistency and add
  ...
  Q17's context-rot problem at a larger scale).
- **Prevention:** When designing a multi-agent decomposition (Q26), explicitly map out which
...
- **Diagnosis:** This is Q16's tool-description-overlap problem at scale — as tools accumulate
...
- **Resolution:** Audit the full tool set for overlap and consolidate where genuinely redundant
  (Q16), and consider whether some tools should be split across multiple, more narrowly-scoped MCP
...
- **Diagnosis:** This is a least-privilege gap that happened to not yet cause harm (Q13, S10) —
...
- **Example:**
  ...
  Access review finds: research-agent has "execute_sql" write access, provisioned
    18 months ago.
  Audit logs (Q23): zero write operations from this agent, ever — only read queries.
...
- **Symptoms:** An agent's final summary states a task succeeded (a file was updated, a record
  ...
  took effect.
- **Diagnosis:** Check the specific tool call in the session trace (Q23) — a common cause is a
